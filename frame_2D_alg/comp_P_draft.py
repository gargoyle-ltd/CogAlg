...
def comp_P(ortho, P, _P, DdX):  # forms vertical derivatives of P params, and conditional ders from norm and DIV comp
    ...
    dM = M - _M;
    mM = min(M, _M)  # no Mx, My: non-core, lesser and redundant bias?

    dDx = abs(Dx) - abs(_Dx);
    mDx = min(abs(Dx), abs(_Dx))  # same-sign Dx in vxP
    dDy = Dy - _Dy;
    mDy = min(Dy, _Dy)  # Dy per sub_P by intra_comp(dx), vs. less vertically specific dI
...
def cluster_P_(val_PP_, blob, Ave, xD):  # scan of vertical Py_ -> comp_P -> 2D mPPs and dPPs, recursive?
    ...
    if val_PP_ * ((max_x - min_x + 1) / (max_y - min_y + 1)) * (
            max(abs(Dx), abs(Dy)) / min(abs(Dx), abs(Dy))) > flip_ave:
        flip(blob)
    ...
    if xD / Ly * val_PP_ > Ave:
        ort = 1  # estimate params of Ps orthogonal to long axis, seg-wide for same-syntax comp_P
    else:
        ort = 0  # to max ave ders, or if xDd to min Pd?
...
def form_PP(typ, P, PP):  # increments continued vPPs or dPPs (not pPs): incr_blob + P_ders?
    ...
    Py_.appendleft((
                   s, ix, x, I, D, Dy, M, My, G, oG, Olp, t2_, Pm, Pd, mx, dx, mL, dL, mI, dI, mD, dD, mDy, dDy, mM, dM,
                   mMy, dMy, div_f, nvars))
...
def scan_params(typ, PP):  # at term_network, term_blob, or term_PP: + P_ders and nvars?
    ...
    for P in P_:  # repack ders into par_s by parameter type:
        ...
        pars_ = [(x, mx, dx), (len(t2_), mL, dL), (I, mI, dI), (D, mD, dD), (Dy, mDy, dDy), (M, mM, dM),
                 (My, mMy, dMy)]  # no nvars?
